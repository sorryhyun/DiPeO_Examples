...
export const spacing = {
  ...
  80: '20rem',    // 320px
  96: '24rem',    // 384px
  // Named spacing sizes for convenience
  xs: '0.25rem',  // 4px - extra small
  sm: '0.5rem',   // 8px - small
  md: '1rem',     // 16px - medium
  lg: '1.5rem',   // 24px - large
  xl: '2rem',     // 32px - extra large
  '2xl': '3rem',  // 48px - 2x large
  '3xl': '4rem',  // 64px - 3x large
} as const;
...
export type Theme = typeof tokens & { variants: typeof themeVariants };
...
export const themeUtils = {
  ...
  getColor(path: string, fallback?: string): string {
    ...
    const colorValue = (colorScale as any)[shade];
    return (typeof colorValue === 'string' ? colorValue : null) || fallback || colors.neutral[500];
  },
