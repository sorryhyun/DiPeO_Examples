...
export interface User {
  ...
  readonly role: Role; // Keep both for backward compatibility
...
export function isPatient(user: User): user is Patient {
  return (user.roles as Role[]).includes('patient');
}

export function isDoctor(user: User): user is Doctor {
  return (user.roles as Role[]).includes('doctor');
}

export function isNurse(user: User): user is Nurse {
  return (user.roles as Role[]).includes('nurse');
}

export function isAdmin(user: User): boolean {
  return (user.roles as Role[]).includes('admin');
}
