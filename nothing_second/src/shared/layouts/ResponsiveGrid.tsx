...
export function ResponsiveGrid({
  ...
  const { columns, columnWidth } = useMemo(() => {
    ...
    // Map responsive hook breakpoints to grid breakpoints  
    const breakpointMapping: Record<'mobile' | 'tablet' | 'desktop' | 'wide', number> = {
      mobile: 1,
      tablet: 2,
      desktop: 3,
      wide: Math.min(maxColumns, maxPossibleColumns),
    };

    const baseColumns = breakpointMapping[breakpoint] || breakpointMapping.desktop;
  ...
  const getItemSpan = (item: GridItem): number => {
    ...
    // Map current breakpoint to grid system keys
    const spanKeyMapping: Record<'mobile' | 'tablet' | 'desktop' | 'wide', keyof typeof item.span> = {
      mobile: 'xs',
      tablet: 'sm', 
      desktop: 'md',
      wide: 'lg',
    ...
    const currentSpanKey = spanKeyMapping[breakpoint];
    const span = item.span[currentSpanKey] || item.span.lg || item.span.md || item.span.sm || item.span.xs || 1;
        ...
        return (
          <div
            ...
            style={{
              ...
              minWidth: 'minWidth' in item ? item.minWidth : undefined,
              maxWidth: 'maxWidth' in item ? item.maxWidth : undefined,
...
export function useResponsiveGrid() {
  ...
  const getOptimalColumns = (
    ...
    const breakpointMapping: Record<'mobile' | 'tablet' | 'desktop' | 'wide', number> = {
      ...
      tablet: 2,
      desktop: 3,
      wide: Math.min(maxColumns, maxPossible),
    };
    
    const baseColumns = breakpointMapping[breakpoint] || 4;
