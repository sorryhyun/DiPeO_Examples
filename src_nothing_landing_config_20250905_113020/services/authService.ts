...
export const authService: AuthService = {
  async login(credentials: LoginRequest): Promise<ApiResult<LoginResponse>> {
    try {
      ...
      if (config.shouldUseMockData && config.mock?.currentUser) {
        const mockUser = config.mock.currentUser;
  ...
  async refreshToken(): Promise<ApiResult<AuthTokens>> {
    ...
    refreshPromise = (async (): Promise<ApiResult<AuthTokens>> => {
      try {
        ...
        // Real API call
        const result = await apiClient.post<RefreshTokenResponse>('/auth/refresh', {
          refreshToken: tokens.refreshToken,
        });
        ...
        return result as ApiResult<AuthTokens>;
      } catch (error) {
